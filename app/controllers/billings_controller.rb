class BillingsController < ApplicationController
  ...
  def create_card
    respond_to do |format|
      if current_trader.stripe_id.nil?
        customer = Stripe::Customer.create({"email": current_trader.email})
        #here we are creating a stripe customer with the help of the Stripe library and pass as parameter email.
        current_trader.update(:stripe_id => customer.id)
        #we are updating current_trader and giving to it stripe_id which is equal to id of customer on Stripe
      end
      ...
      customer = Stripe::Customer.new current_trader.stripe_id
  ...
  def delete_sub
    Stripe::Subscription.delete(Stripe::Customer.retrieve({id: current_trader.stripe_id, expand: ["subscriptions"],})["subscriptions"]["data"][0]["id"])
    redirect_to root_path
  end
